...
public class Solution789_1 {

    /**
     * 曼哈顿距离
     * <p>
     * 问题简化为计算玩家和目的地的曼哈顿距离以及每个阻碍者和目的地的曼哈顿距离，判断玩家是否
     * 可以在阻碍者之前到达目的地。
    ...
    public boolean escapeGhosts(int[][] ghosts, int[] target) {
        int dis = getDistance(new int[]{0, 0}, target);
        for (int[] ghost : ghosts) {
            if (getDistance(ghost, target) <= dis) return false;
        }
        return true;
    }

    private int getDistance(int[] arr1, int[] arr2) {
