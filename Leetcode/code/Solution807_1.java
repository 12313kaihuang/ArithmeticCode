 ...
 */
public class Solution807_1 {

    //贪心
    public int maxIncreaseKeepingSkyline(int[][] grid) {
        int added = 0, n = grid.length;
        int[][] max = new int[2][n];
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                //记录第i行中最大第值
                max[0][i] = Math.max(max[0][i], grid[i][j]);
                max[1][j] = Math.max(max[1][j], grid[i][j]);
            }
        }
//        System.out.println(Arrays.deepToString(max));
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                int t = Math.min(max[0][i], max[1][j]);
                added += Math.abs(grid[i][j] - t);
                grid[i][j] = t;
            }
        }
//        System.out.println(Arrays.deepToString(grid));
    ...
    public static void main(String[] args) {
        System.out.println(new Solution807_1().maxIncreaseKeepingSkyline(new int[][]{
                {3, 0, 8, 4},
                {2, 4, 5, 7},
                {9, 2, 6, 3},
                {0, 3, 1, 0}
